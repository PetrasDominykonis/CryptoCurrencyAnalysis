...
from graphs.ScatterGraph import draw_graph
from datetime import timedelta as td
import datetime as dt

class ScatterGraphView:
    def __init__(self, root, frame):
        ...
        self.ComparingToRadioButtonSelection = IntVar()
        self.ComparingToRadioButtonSelection.set(1)
        ...
        #Start of "compare to" radio buttons
        self.label5 = Label(self.frame,text="Pasirinkite su kuo lyginsime viršuje pasirinkta valiutą / rinką(kompanija):",
                            anchor="w", height=2, width=50)
        self.label5.grid(row=7,column=1,columnspan=3,padx=20,pady=10)
        self.CompareToBitcoinPrice_RB = Radiobutton(self.frame,text="Lyginti su Bitcoin kainomis",height=1,width=30,
                                                    variable=self.ComparingToRadioButtonSelection, value=1)
        self.CompareToBitcoinPrice_RB.grid(row=8,column=1,columnspan=2,padx=5,pady=10)
        self.CompareToItSelfVolume_RB = Radiobutton(self.frame,text="Lyg. su virš. pasirink. pardavimų kiekiais",
                                                    height=1,width=30,variable=self.ComparingToRadioButtonSelection,
                                                    value=2)
        self.CompareToItSelfVolume_RB.grid(row=8,column=3,columnspan=2,padx=5,pady=10)
    ...
    def InstructionsButtonFuncton(self):
        draw_graph(self.RadioButtonSelection.get(),
                   self.ComparingToRadioButtonSelection.get(),

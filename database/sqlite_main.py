...
import pandas as pd
from api_caller import pull_crypto_data_from_api, pull_stocks_data_from_api
from sqlalchemy import create_engine
from os.path import exists

db_file_path = 'price_history.db'
...
def fetch_data_from_db(table_name=None, items=None, start=None, end=None):
    ...
    cursor.execute(f"""SELECT {columns_filter(items)} FROM {str(table_name)} {timeframe}""")
...
# SELECT date,BTC FROM crypto_price_history WHERE date>date('2021-12-01') AND date<date('2021-12-10')
def date_filter(start, end):
    if start != None and end != None:
        ...
        return f"WHERE date>date('{str(start)}')"


def add_data_to_db(table_name, data):
    engine = create_engine('sqlite:///price_history.db', echo=False)
...
create_tables()
refresh_crypto_db_data()
refresh_stocks_db_data()
# def pull_crypto_data_from_api(start=dt.date(2015,1,1), end=dt.date.today()):
fetch_data_from_db("stocks_price_history",["TSLA","GSPC"],dt.date(2021,12,1),dt.date.today())
